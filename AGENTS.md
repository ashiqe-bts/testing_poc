...
Check the current Git branch (root, bts_backend and bts_frontend), and abort this entire process if there are any uncommitted, unstaged, or untracked files in the working directory. Tell the user to commit or stash changes before proceeding, and DO NOT GO ANY FURTHER.
...
Before making any content, switch to a new Git branch (root, bts_backend and bts_frontend) using the `branch_name` derived from the `$ARGUMENTS`. If the branch name is already taken, then append a version number to it: e.g. `feature/card-component-01`

## Step 4. Draft the spec content

Create a markdown spec document that Plan mode can use directly and save it in the \_spec folder using the `feature_slug`. Use the exact structure as defined in the spec template file here: @\_spec/template.md. Do not add technical implementation details such as code examples.
...
description: Plan the feature described in spec
allowed-tools: Read, Write, Glob, Bash(git switch:\*)

---

Plan the feature described in this spec and save it to @\_plan/<feature-name>.md. Do not implement anything.
